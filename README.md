...
#### 3.2 권장 Fact 집계 후보(요구사항)
- `AGG_DAILY_TOTAL` : 일별 전체 요약
- `AGG_DAILY_SELLER` : 일별 판매자 정산
- `AGG_DAILY_CATEGORY` : 일별 카테고리 매출
- `AGG_DAILY_PAYMENT_TYPE` : 일별 결제수단 통계
- `AGG_DAILY_REGION` : 일별 지역 통계(고객/판매자 타입 분리)
- (선택) `AGG_DAILY_REVIEW` : 일별 리뷰/품질 지표
...
- 처리 방식:
  - 기준일별 집계(일 단위)
  - 최근 N일 재집계(지연 데이터 대응) 옵션
- 결제 배분 규칙을 명시하고 재현 가능해야 함(필수)
